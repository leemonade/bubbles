    ...
    wrapper(input, _s) {
      return `<tags class='${_s.classNames.namespace} ${
        _s.mode ? `${_s.classNames[`${_s.mode}Mode`]}` : ''
                    ...
                    tabIndex='-1'>
            <span ${!_s.readonly || _s.mode != 'mix' ? 'contenteditable' : ''} data-placeholder='${
        _s.placeholder || '&#8203;'
    ...
    removeTags(tagElms, silent, tranDuration) {
      ...
      tagElms =
        tagElms && tagElms instanceof HTMLElement
          ...
          : tagElms
          ? [tagElms]
          : [this.getLastTag()]; // normalize tagElms array values:
      ...
      this.settings.hooks
        ...
        .then(() => {
          ...
          else tagsToRemove.forEach(removeNode.bind(this)); // update state regardless of animation
      ...
      function iterateChildren(rootNode) {
        rootNode.childNodes.forEach((node) => {
          if (node.nodeType == 1) {
            ...
            if (
              node.tagName == 'BR' &&
              (node.parentNode == that.DOM.input || node.parentNode.childNodes.length == 1)
            ) {
              result += '\r\n';
